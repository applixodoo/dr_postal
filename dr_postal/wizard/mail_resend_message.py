...
class MailResendMessage(models.TransientModel):
    ...
    def _compute_can_cancel(self):
        for wizard in self:
            wizard.can_cancel = bool(wizard.partner_ids.filtered(lambda p: not p.resend))

    @api.depends('partner_ids.resend')
    def _compute_can_resend(self):
        for wizard in self:
            wizard.can_resend = any([partner.resend for partner in wizard.partner_ids])

    def _compute_partner_readonly(self):
        for wizard in self:
            wizard.partner_readonly = not self.env['res.partner'].check_access_rights('write', raise_exception=False)

    @api.model
    def default_get(self, fields_list):
        rec = super(MailResendMessage, self).default_get(fields_list)
        ...
        if message_id:
            ...
            notification_ids = mail_message_id.notification_ids.filtered(
                lambda notif: notif.notification_type == 'email' and 
                notif.notification_status in ('exception', 'bounce')
            )
            partner_values = [{
    ...
    def resend_mail_action(self):
        ...
        for wizard in self:
            ...
            notif_to_cancel = wizard.notification_ids.filtered(
                lambda notif: notif.notification_type == 'email' and 
                notif.res_partner_id in to_cancel and 
                notif.notification_status in ('exception', 'bounce')
            )
    ...
    def cancel_mail_action(self):
        for wizard in self:
            for notif in wizard.notification_ids:
                notif.filtered(
                    lambda n: n.notification_type == 'email' and 
                    n.notification_status in ('exception', 'bounce')
...
class PartnerResend(models.TransientModel):
    ...
    def action_open_resend_partner(self):
        self.ensure_one()
        action = self.env['ir.actions.act_window']._for_xml_id('dr_postal.mail_resend_partner_action')
